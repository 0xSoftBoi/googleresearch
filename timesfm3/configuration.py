...
class TimesFM3Config:
    ...
    Attributes:
        ...
        max_context_patches: Maximum number of context patches supported by
            the learned temporal machinery (rotary embeddings are unbounded,
            this only bounds the causal mask cache).
        max_horizon_patches: Maximum number of contiguous masked patches the
            model decodes in a single forward pass.
    """
    ...
    def base(cls) -> "TimesFM3Config":
        """~330M parameter configuration (24 alternating layers, d=1024)."""
        return cls()

    @classmethod
    def small(cls) -> "TimesFM3Config":
        """A small configuration for tests, demos and CPU experimentation."""
        return cls(
            model_dim=256,
            num_layers=6,
            num_heads=8,
